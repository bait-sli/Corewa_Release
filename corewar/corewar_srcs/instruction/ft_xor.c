...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/17 14:41:01 by atgerard          #+#    #+#             */
/*   Updated: 2018/04/25 23:04:21 by atgerard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static void			xor(t_process *ec, t_env *env)
{
	int				t[2];
	uint8_t			b[4];
	int				i;
	...
	if (env->vr.verbose == 1)
		ft_printf("P   X | xor %d %d r%d\n", t[0], t[1], ec->par.p[2]);
	ec->pc = (ec->pc + ec->par.fsize) % MEM_SIZE;
}

int					ft_xor(t_process *ec, t_env *env)
{
	uint8_t			opcode;
